...
/***/ function(module, exports, __webpack_require__) {
	...
	var onNewGame = function onNewGame(event) {
	  ...
	  event.preventDefault();
	  api.createGame(store.gameInProgress).then(ui.createGameSuccess).catch(ui.failure);

	  api.getGameId().then(ui.getGameIdSuccess).catch(ui.failure);
	};

	var processTurn = function processTurn(event) {
	  if (store.gameInProgress != undefined) {
	    var clickedSpace = event.target.id.split('space')[1];
	...
	var onLoadHotseat = function onLoadHotseat(event) {
	  var data = getFormFields(this);
	  event.preventDefault();
	  api.loadGameSession(data).then(ui.loadGameSuccess).catch(ui.failure);
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var loadGameSession = function loadGameSession(data) {
	  return $.ajax({
	...
	module.exports = {
	  ...
	  getGameId: getGameId,
	  loadGameSession: loadGameSession
	};
	/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(2)))
...
/***/ function(module, exports, __webpack_require__) {
	...
	var turnSuccess = function turnSuccess() {};
	...
	var getGameIdSuccess = function getGameIdSuccess(data) {
	  var gameId = void 0;
	  var games = [0, 0]; //played, won
	  console.log(data);
	  for (var i in data.games) {
	...
	var loadGameSuccess = function loadGameSuccess(data) {
	  if (!data.game.over) {
	    ...
	    var xVO = 0;
	    for (var i in store.gameInProgress.game.cells) {
	...
	module.exports = {
	  ...
	  getGameIdSuccess: getGameIdSuccess,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var newGame = function newGame(gameType) {
	  for (var i = 0; i < 9; i++) {
	  ...
	  if (gameType === "Hot-Seat") {
	    gameData.game.player_o.email = "guest";
	  }
	  store.gameInProgress = gameData;
	};

	var checkWin = function checkWin() {
	  var gameGrid = store.gameInProgress.game.cells;
	  for (var i = 1; i < 5; i++) {
	    if (i !== 2) {
	...
	var checkPastWins = function checkPastWins(data) {
	  var gameGrid = data;
	  for (var i = 1; i < 5; i++) {
	    if (i !== 2) {
	      if (gameGrid[0] === gameGrid[0 + i] && gameGrid[0] === gameGrid[0 + 2 * i] && gameGrid[0] != "") {
